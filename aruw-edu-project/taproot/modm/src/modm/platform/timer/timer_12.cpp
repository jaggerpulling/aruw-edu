...
// ----------------------------------------------------------------------------
void
modm::platform::Timer12::clockEnable()
{
	enable();
}
...
{
	Rcc::enable<Peripheral::Tim12>();
}

void
modm::platform::Timer12::disable()
{
	TIM12->CR1 = 0;
	TIM12->DIER = 0;
	TIM12->CCER = 0;
		...
		bool limitUpdateEventRequestSource)
{
	...
	if (enableOnePulseMode) {
		TIM12->CR1 = cr1 | TIM_CR1_OPM;
	} else {
		TIM12->CR1 = cr1;
	}
	TIM12->CR2 = static_cast<uint32_t>(masterMode);
	TIM12->SMCR = static_cast<uint32_t>(slaveMode)
		...
		bool xor_ch1_3)
{
	...
	// disable channel
	TIM12->CCER &= ~((TIM_CCER_CC1NP | TIM_CCER_CC1P | TIM_CCER_CC1E) << (channel * 4));
...
// ----------------------------------------------------------------------------
void
modm::platform::Timer12::enableInterruptVector(bool enable, uint32_t priority)
{
	if (enable)
	{
...
// ----------------------------------------------------------------------------
bool
modm::platform::Timer12::isChannelConfiguredAsInput(uint32_t channel)
{
	bool isInput = false;
	switch (channel) {
		case 1:
			isInput = TIM12->CCMR1 & TIM_CCMR1_CC1S;
			break;
		case 2:
			isInput = TIM12->CCMR1 & TIM_CCMR1_CC2S;
			break;
		default:
			break;
	}
