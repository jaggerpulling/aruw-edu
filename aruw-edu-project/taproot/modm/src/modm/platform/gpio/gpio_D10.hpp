...
#ifndef MODM_STM32_GPIO_PIN_D10_HPP
#define MODM_STM32_GPIO_PIN_D10_HPP
...
class GpioD10 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioD10>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioD10;
	using Input = GpioD10;
	using IO = GpioD10;
	using Type = GpioD10;
...
public:
	...
	using Ck = GpioSignal;
	/// Connect to Fmc
	using D15 = GpioSignal;
	/// Connect to Fmc
	using Da15 = GpioSignal;
	/// @}
#endif
	/// @cond
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioD10::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ck { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart3),
			"GpioD10::Ck only connects to Usart3!");
	};
	template< Peripheral peripheral >
	struct D15 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD10::D15 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da15 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD10::Da15 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioD10::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioD10;
...
template<>
struct GpioD10::Ck<Peripheral::Usart3>
{
	using Gpio = GpioD10;
...
template<>
struct GpioD10::D15<Peripheral::Fmc>
{
	using Gpio = GpioD10;
...
template<>
struct GpioD10::Da15<Peripheral::Fmc>
{
	using Gpio = GpioD10;
...
#endif // MODM_STM32_GPIO_PIN_D10_HPP