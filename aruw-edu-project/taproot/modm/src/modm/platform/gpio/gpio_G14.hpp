...
#ifndef MODM_STM32_GPIO_PIN_G14_HPP
#define MODM_STM32_GPIO_PIN_G14_HPP
...
class GpioG14 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioG14>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioG14;
	using Input = GpioG14;
	using IO = GpioG14;
	using Type = GpioG14;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioG14::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct A25 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG14::A25 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Mosi { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi6),
			"GpioG14::Mosi only connects to Spi6!");
	};
	template< Peripheral peripheral >
	struct Tx { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart6),
			"GpioG14::Tx only connects to Usart6!");
	};
	template< Peripheral peripheral >
	struct Txd1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Eth),
			"GpioG14::Txd1 only connects to Eth!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioG14::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioG14;
...
template<>
struct GpioG14::A25<Peripheral::Fmc>
{
	using Gpio = GpioG14;
...
template<>
struct GpioG14::Mosi<Peripheral::Spi6>
{
	using Gpio = GpioG14;
...
template<>
struct GpioG14::Tx<Peripheral::Usart6>
{
	using Gpio = GpioG14;
...
template<>
struct GpioG14::Txd1<Peripheral::Eth>
{
	using Gpio = GpioG14;
...
#endif // MODM_STM32_GPIO_PIN_G14_HPP