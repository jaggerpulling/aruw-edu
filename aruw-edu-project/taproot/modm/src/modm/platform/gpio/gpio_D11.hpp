...
#ifndef MODM_STM32_GPIO_PIN_D11_HPP
#define MODM_STM32_GPIO_PIN_D11_HPP
...
class GpioD11 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioD11>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioD11;
	using Input = GpioD11;
	using IO = GpioD11;
	using Type = GpioD11;
...
public:
	...
	/// Connect to Fmc
	using A16 = GpioSignal;
	/// Connect to Fmc
	using Cle = GpioSignal;
	/// Connect to Usart3
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioD11::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct A16 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD11::A16 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Cle { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD11::Cle only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Cts { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart3),
			"GpioD11::Cts only connects to Usart3!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioD11::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioD11;
...
template<>
struct GpioD11::A16<Peripheral::Fmc>
{
	using Gpio = GpioD11;
...
template<>
struct GpioD11::Cle<Peripheral::Fmc>
{
	using Gpio = GpioD11;
...
template<>
struct GpioD11::Cts<Peripheral::Usart3>
{
	using Gpio = GpioD11;
...
#endif // MODM_STM32_GPIO_PIN_D11_HPP