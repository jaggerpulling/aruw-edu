...
#ifndef MODM_STM32_GPIO_PIN_E2_HPP
#define MODM_STM32_GPIO_PIN_E2_HPP
...
class GpioE2 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioE2>;
	friend class Adc;
...
public:
	...
	/// Connect to Fmc
	using A23 = GpioSignal;
	/// Connect to Sai1
	using Mclka = GpioSignal;
	/// Connect to Spi4
	using Sck = GpioSignal;
	/// Connect to Sys
	using Traceclk = GpioSignal;
	/// Connect to Eth
	using Txd3 = GpioSignal;
	/// @}
#endif
	/// @cond
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioE2::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct A23 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE2::A23 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Mclka { static void connect();
		static_assert(
			(peripheral == Peripheral::Sai1),
			"GpioE2::Mclka only connects to Sai1!");
	};
	template< Peripheral peripheral >
	struct Sck { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi4),
			"GpioE2::Sck only connects to Spi4!");
	};
	template< Peripheral peripheral >
	struct Traceclk { static void connect();
		static_assert(
			(peripheral == Peripheral::Sys),
			"GpioE2::Traceclk only connects to Sys!");
	};
	template< Peripheral peripheral >
	struct Txd3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Eth),
			"GpioE2::Txd3 only connects to Eth!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioE2::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioE2;
...
template<>
struct GpioE2::A23<Peripheral::Fmc>
{
	using Gpio = GpioE2;
...
template<>
struct GpioE2::Mclka<Peripheral::Sai1>
{
	using Gpio = GpioE2;
...
template<>
struct GpioE2::Sck<Peripheral::Spi4>
{
	using Gpio = GpioE2;
...
template<>
struct GpioE2::Traceclk<Peripheral::Sys>
{
	using Gpio = GpioE2;
...
template<>
struct GpioE2::Txd3<Peripheral::Eth>
{
	using Gpio = GpioE2;
...
#endif // MODM_STM32_GPIO_PIN_E2_HPP