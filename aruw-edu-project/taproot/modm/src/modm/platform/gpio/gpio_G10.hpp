...
#ifndef MODM_STM32_GPIO_PIN_G10_HPP
#define MODM_STM32_GPIO_PIN_G10_HPP
...
class GpioG10 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioG10>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioG10;
	using Input = GpioG10;
	using IO = GpioG10;
	using Type = GpioG10;
...
public:
	...
	using D2 = GpioSignal;
	/// Connect to Fmc
	using Nce41 = GpioSignal;
	/// Connect to Fmc
	using Ne3 = GpioSignal;
	/// @}
#endif
	/// @cond
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioG10::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct D2 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioG10::D2 only connects to Dcmi!");
	};
	template< Peripheral peripheral >
	struct Nce41 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG10::Nce41 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Ne3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG10::Ne3 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioG10::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioG10;
...
template<>
struct GpioG10::D2<Peripheral::Dcmi>
{
	using Gpio = GpioG10;
...
template<>
struct GpioG10::Nce41<Peripheral::Fmc>
{
	using Gpio = GpioG10;
...
template<>
struct GpioG10::Ne3<Peripheral::Fmc>
{
	using Gpio = GpioG10;
...
#endif // MODM_STM32_GPIO_PIN_G10_HPP