...
#ifndef MODM_STM32_GPIO_PIN_D1_HPP
#define MODM_STM32_GPIO_PIN_D1_HPP
...
class GpioD1 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioD1>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioD1::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct D3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD1::D3 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD1::Da3 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Tx { static void connect();
		static_assert(
			(peripheral == Peripheral::Can1),
			"GpioD1::Tx only connects to Can1!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioD1::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioD1;
...
template<>
struct GpioD1::D3<Peripheral::Fmc>
{
	using Gpio = GpioD1;
...
template<>
struct GpioD1::Da3<Peripheral::Fmc>
{
	using Gpio = GpioD1;
...
template<>
struct GpioD1::Tx<Peripheral::Can1>
{
	using Gpio = GpioD1;
...
#endif // MODM_STM32_GPIO_PIN_D1_HPP