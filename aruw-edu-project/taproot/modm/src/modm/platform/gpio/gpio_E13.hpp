...
#ifndef MODM_STM32_GPIO_PIN_E13_HPP
#define MODM_STM32_GPIO_PIN_E13_HPP
...
class GpioE13 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioE13>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioE13;
	using Input = GpioE13;
	using IO = GpioE13;
	using Type = GpioE13;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioE13::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim1),
			"GpioE13::Ch3 only connects to Tim1!");
	};
	template< Peripheral peripheral >
	struct D10 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE13::D10 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da10 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE13::Da10 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Miso { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi4),
			"GpioE13::Miso only connects to Spi4!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioE13::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioE13;
...
template<>
struct GpioE13::Ch3<Peripheral::Tim1>
{
	using Gpio = GpioE13;
...
template<>
struct GpioE13::D10<Peripheral::Fmc>
{
	using Gpio = GpioE13;
...
template<>
struct GpioE13::Da10<Peripheral::Fmc>
{
	using Gpio = GpioE13;
...
template<>
struct GpioE13::Miso<Peripheral::Spi4>
{
	using Gpio = GpioE13;
...
#endif // MODM_STM32_GPIO_PIN_E13_HPP