...
#ifndef MODM_STM32_GPIO_PIN_E6_HPP
#define MODM_STM32_GPIO_PIN_E6_HPP
...
class GpioE6 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioE6>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioE6::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct A22 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE6::A22 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Ch2 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim9),
			"GpioE6::Ch2 only connects to Tim9!");
	};
	template< Peripheral peripheral >
	struct D7 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioE6::D7 only connects to Dcmi!");
	};
	template< Peripheral peripheral >
	struct Mosi { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi4),
			"GpioE6::Mosi only connects to Spi4!");
	};
	template< Peripheral peripheral >
	struct Sda { static void connect();
		static_assert(
			(peripheral == Peripheral::Sai1),
			"GpioE6::Sda only connects to Sai1!");
	};
	template< Peripheral peripheral >
	struct Traced3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Sys),
			"GpioE6::Traced3 only connects to Sys!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioE6::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioE6;
...
template<>
struct GpioE6::A22<Peripheral::Fmc>
{
	using Gpio = GpioE6;
...
template<>
struct GpioE6::Ch2<Peripheral::Tim9>
{
	using Gpio = GpioE6;
...
template<>
struct GpioE6::D7<Peripheral::Dcmi>
{
	using Gpio = GpioE6;
...
template<>
struct GpioE6::Mosi<Peripheral::Spi4>
{
	using Gpio = GpioE6;
...
template<>
struct GpioE6::Sda<Peripheral::Sai1>
{
	using Gpio = GpioE6;
...
template<>
struct GpioE6::Traced3<Peripheral::Sys>
{
	using Gpio = GpioE6;
...
#endif // MODM_STM32_GPIO_PIN_E6_HPP