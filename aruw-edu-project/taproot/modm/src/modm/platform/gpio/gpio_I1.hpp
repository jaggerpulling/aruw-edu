...
#ifndef MODM_STM32_GPIO_PIN_I1_HPP
#define MODM_STM32_GPIO_PIN_I1_HPP
...
class GpioI1 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioI1>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioI1::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ck { static void connect();
		static_assert(
			(peripheral == Peripheral::I2s2),
			"GpioI1::Ck only connects to I2s2!");
	};
	template< Peripheral peripheral >
	struct D25 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioI1::D25 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct D8 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioI1::D8 only connects to Dcmi!");
	};
	template< Peripheral peripheral >
	struct Sck { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi2),
			"GpioI1::Sck only connects to Spi2!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioI1::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioI1;
...
template<>
struct GpioI1::Ck<Peripheral::I2s2>
{
	using Gpio = GpioI1;
...
template<>
struct GpioI1::D25<Peripheral::Fmc>
{
	using Gpio = GpioI1;
...
template<>
struct GpioI1::D8<Peripheral::Dcmi>
{
	using Gpio = GpioI1;
...
template<>
struct GpioI1::Sck<Peripheral::Spi2>
{
	using Gpio = GpioI1;
...
#endif // MODM_STM32_GPIO_PIN_I1_HPP