...
#ifndef MODM_STM32_GPIO_PIN_H12_HPP
#define MODM_STM32_GPIO_PIN_H12_HPP
...
class GpioH12 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioH12>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioH12;
	using Input = GpioH12;
	using IO = GpioH12;
	using Type = GpioH12;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioH12::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim5),
			"GpioH12::Ch3 only connects to Tim5!");
	};
	template< Peripheral peripheral >
	struct D20 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioH12::D20 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct D3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioH12::D3 only connects to Dcmi!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioH12::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioH12;
...
template<>
struct GpioH12::Ch3<Peripheral::Tim5>
{
	using Gpio = GpioH12;
...
template<>
struct GpioH12::D20<Peripheral::Fmc>
{
	using Gpio = GpioH12;
...
template<>
struct GpioH12::D3<Peripheral::Dcmi>
{
	using Gpio = GpioH12;
...
#endif // MODM_STM32_GPIO_PIN_H12_HPP