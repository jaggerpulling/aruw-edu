...
#ifndef MODM_STM32_GPIO_PIN_H3_HPP
#define MODM_STM32_GPIO_PIN_H3_HPP
...
class GpioH3 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioH3>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioH3::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Col { static void connect();
		static_assert(
			(peripheral == Peripheral::Eth),
			"GpioH3::Col only connects to Eth!");
	};
	template< Peripheral peripheral >
	struct Sdne0 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioH3::Sdne0 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioH3::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioH3;
...
template<>
struct GpioH3::Col<Peripheral::Eth>
{
	using Gpio = GpioH3;
...
template<>
struct GpioH3::Sdne0<Peripheral::Fmc>
{
	using Gpio = GpioH3;
...
#endif // MODM_STM32_GPIO_PIN_H3_HPP