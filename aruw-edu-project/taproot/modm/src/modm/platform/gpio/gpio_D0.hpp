...
#ifndef MODM_STM32_GPIO_PIN_D0_HPP
#define MODM_STM32_GPIO_PIN_D0_HPP
...
class GpioD0 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioD0>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioD0::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct D2 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD0::D2 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da2 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD0::Da2 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Rx { static void connect();
		static_assert(
			(peripheral == Peripheral::Can1),
			"GpioD0::Rx only connects to Can1!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioD0::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioD0;
...
template<>
struct GpioD0::D2<Peripheral::Fmc>
{
	using Gpio = GpioD0;
...
template<>
struct GpioD0::Da2<Peripheral::Fmc>
{
	using Gpio = GpioD0;
...
template<>
struct GpioD0::Rx<Peripheral::Can1>
{
	using Gpio = GpioD0;
...
#endif // MODM_STM32_GPIO_PIN_D0_HPP