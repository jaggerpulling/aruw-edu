...
#ifndef MODM_STM32_GPIO_PIN_D9_HPP
#define MODM_STM32_GPIO_PIN_D9_HPP
...
class GpioD9 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioD9>;
	friend class Adc;
...
public:
	...
	/// Connect to Fmc
	using D14 = GpioSignal;
	/// Connect to Fmc
	using Da14 = GpioSignal;
	/// Connect to Usart3
	using Rx = GpioSignal;
	/// @}
#endif
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioD9::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct D14 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD9::D14 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da14 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD9::Da14 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Rx { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart3),
			"GpioD9::Rx only connects to Usart3!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioD9::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioD9;
...
template<>
struct GpioD9::D14<Peripheral::Fmc>
{
	using Gpio = GpioD9;
...
template<>
struct GpioD9::Da14<Peripheral::Fmc>
{
	using Gpio = GpioD9;
...
template<>
struct GpioD9::Rx<Peripheral::Usart3>
{
	using Gpio = GpioD9;
...
#endif // MODM_STM32_GPIO_PIN_D9_HPP