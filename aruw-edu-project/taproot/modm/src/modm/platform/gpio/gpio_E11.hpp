...
#ifndef MODM_STM32_GPIO_PIN_E11_HPP
#define MODM_STM32_GPIO_PIN_E11_HPP
...
class GpioE11 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioE11>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioE11;
	using Input = GpioE11;
	using IO = GpioE11;
	using Type = GpioE11;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioE11::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch2 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim1),
			"GpioE11::Ch2 only connects to Tim1!");
	};
	template< Peripheral peripheral >
	struct D8 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE11::D8 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da8 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE11::Da8 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Nss { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi4),
			"GpioE11::Nss only connects to Spi4!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioE11::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioE11;
...
template<>
struct GpioE11::Ch2<Peripheral::Tim1>
{
	using Gpio = GpioE11;
...
template<>
struct GpioE11::D8<Peripheral::Fmc>
{
	using Gpio = GpioE11;
...
template<>
struct GpioE11::Da8<Peripheral::Fmc>
{
	using Gpio = GpioE11;
...
template<>
struct GpioE11::Nss<Peripheral::Spi4>
{
	using Gpio = GpioE11;
...
#endif // MODM_STM32_GPIO_PIN_E11_HPP