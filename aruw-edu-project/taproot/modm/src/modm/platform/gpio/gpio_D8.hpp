...
#ifndef MODM_STM32_GPIO_PIN_D8_HPP
#define MODM_STM32_GPIO_PIN_D8_HPP
...
class GpioD8 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioD8>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioD8::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct D13 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD8::D13 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da13 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD8::Da13 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Tx { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart3),
			"GpioD8::Tx only connects to Usart3!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioD8::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioD8;
...
template<>
struct GpioD8::D13<Peripheral::Fmc>
{
	using Gpio = GpioD8;
...
template<>
struct GpioD8::Da13<Peripheral::Fmc>
{
	using Gpio = GpioD8;
...
template<>
struct GpioD8::Tx<Peripheral::Usart3>
{
	using Gpio = GpioD8;
...
#endif // MODM_STM32_GPIO_PIN_D8_HPP