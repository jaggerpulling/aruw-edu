...
#ifndef MODM_STM32_GPIO_PIN_D15_HPP
#define MODM_STM32_GPIO_PIN_D15_HPP
...
class GpioD15 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioD15>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioD15;
	using Input = GpioD15;
	using IO = GpioD15;
	using Type = GpioD15;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioD15::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch4 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim4),
			"GpioD15::Ch4 only connects to Tim4!");
	};
	template< Peripheral peripheral >
	struct D1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD15::D1 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD15::Da1 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioD15::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioD15;
...
template<>
struct GpioD15::Ch4<Peripheral::Tim4>
{
	using Gpio = GpioD15;
...
template<>
struct GpioD15::D1<Peripheral::Fmc>
{
	using Gpio = GpioD15;
...
template<>
struct GpioD15::Da1<Peripheral::Fmc>
{
	using Gpio = GpioD15;
...
#endif // MODM_STM32_GPIO_PIN_D15_HPP