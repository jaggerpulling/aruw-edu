...
#ifndef MODM_STM32_GPIO_PIN_G7_HPP
#define MODM_STM32_GPIO_PIN_G7_HPP
...
class GpioG7 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioG7>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioG7::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ck { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart6),
			"GpioG7::Ck only connects to Usart6!");
	};
	template< Peripheral peripheral >
	struct D13 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioG7::D13 only connects to Dcmi!");
	};
	template< Peripheral peripheral >
	struct Int3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG7::Int3 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioG7::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioG7;
...
template<>
struct GpioG7::Ck<Peripheral::Usart6>
{
	using Gpio = GpioG7;
...
template<>
struct GpioG7::D13<Peripheral::Dcmi>
{
	using Gpio = GpioG7;
...
template<>
struct GpioG7::Int3<Peripheral::Fmc>
{
	using Gpio = GpioG7;
...
#endif // MODM_STM32_GPIO_PIN_G7_HPP