...
#ifndef MODM_STM32_GPIO_PIN_G5_HPP
#define MODM_STM32_GPIO_PIN_G5_HPP
...
class GpioG5 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioG5>;
	friend class Adc;
...
public:
	...
	/// Connect to Fmc
	using A15 = GpioSignal;
	/// Connect to Fmc
	using Ba1 = GpioSignal;
	/// @}
#endif
	/// @cond
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioG5::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct A15 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG5::A15 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Ba1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG5::Ba1 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioG5::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioG5;
...
template<>
struct GpioG5::A15<Peripheral::Fmc>
{
	using Gpio = GpioG5;
...
template<>
struct GpioG5::Ba1<Peripheral::Fmc>
{
	using Gpio = GpioG5;
...
#endif // MODM_STM32_GPIO_PIN_G5_HPP