...
#ifndef MODM_STM32_GPIO_PIN_G15_HPP
#define MODM_STM32_GPIO_PIN_G15_HPP
...
class GpioG15 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioG15>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioG15;
	using Input = GpioG15;
	using IO = GpioG15;
	using Type = GpioG15;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioG15::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Cts { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart6),
			"GpioG15::Cts only connects to Usart6!");
	};
	template< Peripheral peripheral >
	struct D13 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioG15::D13 only connects to Dcmi!");
	};
	template< Peripheral peripheral >
	struct Sdncas { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG15::Sdncas only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioG15::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioG15;
...
template<>
struct GpioG15::Cts<Peripheral::Usart6>
{
	using Gpio = GpioG15;
...
template<>
struct GpioG15::D13<Peripheral::Dcmi>
{
	using Gpio = GpioG15;
...
template<>
struct GpioG15::Sdncas<Peripheral::Fmc>
{
	using Gpio = GpioG15;
...
#endif // MODM_STM32_GPIO_PIN_G15_HPP