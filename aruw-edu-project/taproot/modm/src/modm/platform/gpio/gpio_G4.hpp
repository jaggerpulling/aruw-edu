...
#ifndef MODM_STM32_GPIO_PIN_G4_HPP
#define MODM_STM32_GPIO_PIN_G4_HPP
...
class GpioG4 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioG4>;
	friend class Adc;
...
public:
	...
	/// Connect to Fmc
	using A14 = GpioSignal;
	/// Connect to Fmc
	using Ba0 = GpioSignal;
	/// @}
#endif
	/// @cond
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioG4::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct A14 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG4::A14 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Ba0 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG4::Ba0 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioG4::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioG4;
...
template<>
struct GpioG4::A14<Peripheral::Fmc>
{
	using Gpio = GpioG4;
...
template<>
struct GpioG4::Ba0<Peripheral::Fmc>
{
	using Gpio = GpioG4;
...
#endif // MODM_STM32_GPIO_PIN_G4_HPP