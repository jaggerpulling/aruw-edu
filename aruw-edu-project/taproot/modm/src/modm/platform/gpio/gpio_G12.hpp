...
#ifndef MODM_STM32_GPIO_PIN_G12_HPP
#define MODM_STM32_GPIO_PIN_G12_HPP
...
class GpioG12 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioG12>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioG12;
	using Input = GpioG12;
	using IO = GpioG12;
	using Type = GpioG12;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioG12::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Miso { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi6),
			"GpioG12::Miso only connects to Spi6!");
	};
	template< Peripheral peripheral >
	struct Ne4 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG12::Ne4 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Rts { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart6),
			"GpioG12::Rts only connects to Usart6!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioG12::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioG12;
...
template<>
struct GpioG12::Miso<Peripheral::Spi6>
{
	using Gpio = GpioG12;
...
template<>
struct GpioG12::Ne4<Peripheral::Fmc>
{
	using Gpio = GpioG12;
...
template<>
struct GpioG12::Rts<Peripheral::Usart6>
{
	using Gpio = GpioG12;
...
#endif // MODM_STM32_GPIO_PIN_G12_HPP