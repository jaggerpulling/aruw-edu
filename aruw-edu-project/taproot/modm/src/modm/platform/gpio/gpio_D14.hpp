...
#ifndef MODM_STM32_GPIO_PIN_D14_HPP
#define MODM_STM32_GPIO_PIN_D14_HPP
...
class GpioD14 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioD14>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioD14;
	using Input = GpioD14;
	using IO = GpioD14;
	using Type = GpioD14;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioD14::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim4),
			"GpioD14::Ch3 only connects to Tim4!");
	};
	template< Peripheral peripheral >
	struct D0 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD14::D0 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da0 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD14::Da0 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioD14::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioD14;
...
template<>
struct GpioD14::Ch3<Peripheral::Tim4>
{
	using Gpio = GpioD14;
...
template<>
struct GpioD14::D0<Peripheral::Fmc>
{
	using Gpio = GpioD14;
...
template<>
struct GpioD14::Da0<Peripheral::Fmc>
{
	using Gpio = GpioD14;
...
#endif // MODM_STM32_GPIO_PIN_D14_HPP