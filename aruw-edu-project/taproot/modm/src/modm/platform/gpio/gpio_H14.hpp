...
#ifndef MODM_STM32_GPIO_PIN_H14_HPP
#define MODM_STM32_GPIO_PIN_H14_HPP
...
class GpioH14 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioH14>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioH14;
	using Input = GpioH14;
	using IO = GpioH14;
	using Type = GpioH14;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioH14::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch2n { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim8),
			"GpioH14::Ch2n only connects to Tim8!");
	};
	template< Peripheral peripheral >
	struct D22 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioH14::D22 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct D4 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioH14::D4 only connects to Dcmi!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioH14::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioH14;
...
template<>
struct GpioH14::Ch2n<Peripheral::Tim8>
{
	using Gpio = GpioH14;
...
template<>
struct GpioH14::D22<Peripheral::Fmc>
{
	using Gpio = GpioH14;
...
template<>
struct GpioH14::D4<Peripheral::Dcmi>
{
	using Gpio = GpioH14;
...
#endif // MODM_STM32_GPIO_PIN_H14_HPP