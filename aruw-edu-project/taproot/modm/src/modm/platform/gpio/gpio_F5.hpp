...
#ifndef MODM_STM32_GPIO_PIN_F5_HPP
#define MODM_STM32_GPIO_PIN_F5_HPP
...
class GpioF5 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioF5>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioF5::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct A5 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioF5::A5 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct In15 { static void connect();
		static_assert(
			(peripheral == Peripheral::Adc3),
			"GpioF5::In15 only connects to Adc3!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioF5::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioF5;
...
template<>
struct GpioF5::A5<Peripheral::Fmc>
{
	using Gpio = GpioF5;
...
template<>
struct GpioF5::In15<Peripheral::Adc3>
{
	using Gpio = GpioF5;
...
template<>
constexpr int8_t
GpioF5::AdcChannel<Peripheral::Adc3> = 15;
/// @endcond

} // namespace modm::platform

#endif // MODM_STM32_GPIO_PIN_F5_HPP