...
#ifndef MODM_STM32_GPIO_PIN_D12_HPP
#define MODM_STM32_GPIO_PIN_D12_HPP
...
class GpioD12 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioD12>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioD12;
	using Input = GpioD12;
	using IO = GpioD12;
	using Type = GpioD12;
...
public:
	...
	/// Connect to Fmc
	using A17 = GpioSignal;
	/// Connect to Fmc
	using Ale = GpioSignal;
	/// Connect to Tim4
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioD12::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct A17 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD12::A17 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Ale { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD12::Ale only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Ch1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim4),
			"GpioD12::Ch1 only connects to Tim4!");
	};
	template< Peripheral peripheral >
	struct Rts { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart3),
			"GpioD12::Rts only connects to Usart3!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioD12::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioD12;
...
template<>
struct GpioD12::A17<Peripheral::Fmc>
{
	using Gpio = GpioD12;
...
template<>
struct GpioD12::Ale<Peripheral::Fmc>
{
	using Gpio = GpioD12;
...
template<>
struct GpioD12::Ch1<Peripheral::Tim4>
{
	using Gpio = GpioD12;
...
template<>
struct GpioD12::Rts<Peripheral::Usart3>
{
	using Gpio = GpioD12;
...
#endif // MODM_STM32_GPIO_PIN_D12_HPP