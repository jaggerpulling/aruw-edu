...
#ifndef MODM_STM32_GPIO_PIN_G13_HPP
#define MODM_STM32_GPIO_PIN_G13_HPP
...
class GpioG13 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioG13>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioG13;
	using Input = GpioG13;
	using IO = GpioG13;
	using Type = GpioG13;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioG13::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct A24 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG13::A24 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Cts { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart6),
			"GpioG13::Cts only connects to Usart6!");
	};
	template< Peripheral peripheral >
	struct Sck { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi6),
			"GpioG13::Sck only connects to Spi6!");
	};
	template< Peripheral peripheral >
	struct Txd0 { static void connect();
		static_assert(
			(peripheral == Peripheral::Eth),
			"GpioG13::Txd0 only connects to Eth!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioG13::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioG13;
...
template<>
struct GpioG13::A24<Peripheral::Fmc>
{
	using Gpio = GpioG13;
...
template<>
struct GpioG13::Cts<Peripheral::Usart6>
{
	using Gpio = GpioG13;
...
template<>
struct GpioG13::Sck<Peripheral::Spi6>
{
	using Gpio = GpioG13;
...
template<>
struct GpioG13::Txd0<Peripheral::Eth>
{
	using Gpio = GpioG13;
...
#endif // MODM_STM32_GPIO_PIN_G13_HPP