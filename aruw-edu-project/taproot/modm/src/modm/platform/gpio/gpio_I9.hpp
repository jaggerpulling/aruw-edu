...
#ifndef MODM_STM32_GPIO_PIN_I9_HPP
#define MODM_STM32_GPIO_PIN_I9_HPP
...
class GpioI9 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioI9>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioI9::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct D30 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioI9::D30 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Rx { static void connect();
		static_assert(
			(peripheral == Peripheral::Can1),
			"GpioI9::Rx only connects to Can1!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioI9::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioI9;
...
template<>
struct GpioI9::D30<Peripheral::Fmc>
{
	using Gpio = GpioI9;
...
template<>
struct GpioI9::Rx<Peripheral::Can1>
{
	using Gpio = GpioI9;
...
#endif // MODM_STM32_GPIO_PIN_I9_HPP