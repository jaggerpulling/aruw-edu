...
#ifndef MODM_STM32_GPIO_PIN_F7_HPP
#define MODM_STM32_GPIO_PIN_F7_HPP
...
class GpioF7 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioF7>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioF7::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim11),
			"GpioF7::Ch1 only connects to Tim11!");
	};
	template< Peripheral peripheral >
	struct In5 { static void connect();
		static_assert(
			(peripheral == Peripheral::Adc3),
			"GpioF7::In5 only connects to Adc3!");
	};
	template< Peripheral peripheral >
	struct Mclkb { static void connect();
		static_assert(
			(peripheral == Peripheral::Sai1),
			"GpioF7::Mclkb only connects to Sai1!");
	};
	template< Peripheral peripheral >
	struct Nreg { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioF7::Nreg only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Sck { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi5),
			"GpioF7::Sck only connects to Spi5!");
	};
	template< Peripheral peripheral >
	struct Tx { static void connect();
		static_assert(
			(peripheral == Peripheral::Uart7),
			"GpioF7::Tx only connects to Uart7!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioF7::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioF7;
...
template<>
struct GpioF7::Ch1<Peripheral::Tim11>
{
	using Gpio = GpioF7;
...
template<>
struct GpioF7::In5<Peripheral::Adc3>
{
	using Gpio = GpioF7;
...
template<>
constexpr int8_t
GpioF7::AdcChannel<Peripheral::Adc3> = 5;
template<>
struct GpioF7::Mclkb<Peripheral::Sai1>
{
	using Gpio = GpioF7;
...
template<>
struct GpioF7::Nreg<Peripheral::Fmc>
{
	using Gpio = GpioF7;
...
template<>
struct GpioF7::Sck<Peripheral::Spi5>
{
	using Gpio = GpioF7;
...
template<>
struct GpioF7::Tx<Peripheral::Uart7>
{
	using Gpio = GpioF7;
...
#endif // MODM_STM32_GPIO_PIN_F7_HPP