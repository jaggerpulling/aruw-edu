...
#ifndef MODM_STM32_GPIO_PIN_I7_HPP
#define MODM_STM32_GPIO_PIN_I7_HPP
...
class GpioI7 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioI7>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioI7::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim8),
			"GpioI7::Ch3 only connects to Tim8!");
	};
	template< Peripheral peripheral >
	struct D29 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioI7::D29 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct D7 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioI7::D7 only connects to Dcmi!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioI7::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioI7;
...
template<>
struct GpioI7::Ch3<Peripheral::Tim8>
{
	using Gpio = GpioI7;
...
template<>
struct GpioI7::D29<Peripheral::Fmc>
{
	using Gpio = GpioI7;
...
template<>
struct GpioI7::D7<Peripheral::Dcmi>
{
	using Gpio = GpioI7;
...
#endif // MODM_STM32_GPIO_PIN_I7_HPP