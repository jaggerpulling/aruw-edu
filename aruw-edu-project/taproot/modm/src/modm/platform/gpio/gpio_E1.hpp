...
#ifndef MODM_STM32_GPIO_PIN_E1_HPP
#define MODM_STM32_GPIO_PIN_E1_HPP
...
class GpioE1 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioE1>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioE1::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct D3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioE1::D3 only connects to Dcmi!");
	};
	template< Peripheral peripheral >
	struct Nbl1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE1::Nbl1 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Tx { static void connect();
		static_assert(
			(peripheral == Peripheral::Uart8),
			"GpioE1::Tx only connects to Uart8!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioE1::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioE1;
...
template<>
struct GpioE1::D3<Peripheral::Dcmi>
{
	using Gpio = GpioE1;
...
template<>
struct GpioE1::Nbl1<Peripheral::Fmc>
{
	using Gpio = GpioE1;
...
template<>
struct GpioE1::Tx<Peripheral::Uart8>
{
	using Gpio = GpioE1;
...
#endif // MODM_STM32_GPIO_PIN_E1_HPP