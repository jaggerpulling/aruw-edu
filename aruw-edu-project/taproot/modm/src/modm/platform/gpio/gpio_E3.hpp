...
#ifndef MODM_STM32_GPIO_PIN_E3_HPP
#define MODM_STM32_GPIO_PIN_E3_HPP
...
class GpioE3 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioE3>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioE3::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct A19 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE3::A19 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Sdb { static void connect();
		static_assert(
			(peripheral == Peripheral::Sai1),
			"GpioE3::Sdb only connects to Sai1!");
	};
	template< Peripheral peripheral >
	struct Traced0 { static void connect();
		static_assert(
			(peripheral == Peripheral::Sys),
			"GpioE3::Traced0 only connects to Sys!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioE3::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioE3;
...
template<>
struct GpioE3::A19<Peripheral::Fmc>
{
	using Gpio = GpioE3;
...
template<>
struct GpioE3::Sdb<Peripheral::Sai1>
{
	using Gpio = GpioE3;
...
template<>
struct GpioE3::Traced0<Peripheral::Sys>
{
	using Gpio = GpioE3;
...
#endif // MODM_STM32_GPIO_PIN_E3_HPP