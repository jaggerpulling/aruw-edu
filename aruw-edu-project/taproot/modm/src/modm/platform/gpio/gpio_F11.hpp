...
#ifndef MODM_STM32_GPIO_PIN_F11_HPP
#define MODM_STM32_GPIO_PIN_F11_HPP
...
class GpioF11 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioF11>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioF11;
	using Input = GpioF11;
	using IO = GpioF11;
	using Type = GpioF11;
...
public:
	...
	/// Connect to Dcmi
	using D12 = GpioSignal;
	/// Connect to Spi5
	using Mosi = GpioSignal;
	/// Connect to Fmc
	using Sdnras = GpioSignal;
	/// @}
#endif
	/// @cond
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioF11::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct D12 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioF11::D12 only connects to Dcmi!");
	};
	template< Peripheral peripheral >
	struct Mosi { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi5),
			"GpioF11::Mosi only connects to Spi5!");
	};
	template< Peripheral peripheral >
	struct Sdnras { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioF11::Sdnras only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioF11::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioF11;
...
template<>
struct GpioF11::D12<Peripheral::Dcmi>
{
	using Gpio = GpioF11;
...
template<>
struct GpioF11::Mosi<Peripheral::Spi5>
{
	using Gpio = GpioF11;
...
template<>
struct GpioF11::Sdnras<Peripheral::Fmc>
{
	using Gpio = GpioF11;
...
#endif // MODM_STM32_GPIO_PIN_F11_HPP