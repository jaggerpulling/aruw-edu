...
#ifndef MODM_STM32_GPIO_PIN_F10_HPP
#define MODM_STM32_GPIO_PIN_F10_HPP
...
class GpioF10 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioF10>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioF10;
	using Input = GpioF10;
	using IO = GpioF10;
	using Type = GpioF10;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioF10::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct D11 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioF10::D11 only connects to Dcmi!");
	};
	template< Peripheral peripheral >
	struct In8 { static void connect();
		static_assert(
			(peripheral == Peripheral::Adc3),
			"GpioF10::In8 only connects to Adc3!");
	};
	template< Peripheral peripheral >
	struct Intr { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioF10::Intr only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioF10::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioF10;
...
template<>
struct GpioF10::D11<Peripheral::Dcmi>
{
	using Gpio = GpioF10;
...
template<>
struct GpioF10::In8<Peripheral::Adc3>
{
	using Gpio = GpioF10;
...
template<>
constexpr int8_t
GpioF10::AdcChannel<Peripheral::Adc3> = 8;
template<>
struct GpioF10::Intr<Peripheral::Fmc>
{
	using Gpio = GpioF10;
...
#endif // MODM_STM32_GPIO_PIN_F10_HPP