...
#ifndef MODM_STM32_GPIO_PIN_H13_HPP
#define MODM_STM32_GPIO_PIN_H13_HPP
...
class GpioH13 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioH13>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioH13;
	using Input = GpioH13;
	using IO = GpioH13;
	using Type = GpioH13;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioH13::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch1n { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim8),
			"GpioH13::Ch1n only connects to Tim8!");
	};
	template< Peripheral peripheral >
	struct D21 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioH13::D21 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Tx { static void connect();
		static_assert(
			(peripheral == Peripheral::Can1),
			"GpioH13::Tx only connects to Can1!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioH13::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioH13;
...
template<>
struct GpioH13::Ch1n<Peripheral::Tim8>
{
	using Gpio = GpioH13;
...
template<>
struct GpioH13::D21<Peripheral::Fmc>
{
	using Gpio = GpioH13;
...
template<>
struct GpioH13::Tx<Peripheral::Can1>
{
	using Gpio = GpioH13;
...
#endif // MODM_STM32_GPIO_PIN_H13_HPP