...
#ifndef MODM_STM32_GPIO_PIN_E12_HPP
#define MODM_STM32_GPIO_PIN_E12_HPP
...
class GpioE12 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioE12>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioE12;
	using Input = GpioE12;
	using IO = GpioE12;
	using Type = GpioE12;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioE12::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch3n { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim1),
			"GpioE12::Ch3n only connects to Tim1!");
	};
	template< Peripheral peripheral >
	struct D9 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE12::D9 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da9 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE12::Da9 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Sck { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi4),
			"GpioE12::Sck only connects to Spi4!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioE12::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioE12;
...
template<>
struct GpioE12::Ch3n<Peripheral::Tim1>
{
	using Gpio = GpioE12;
...
template<>
struct GpioE12::D9<Peripheral::Fmc>
{
	using Gpio = GpioE12;
...
template<>
struct GpioE12::Da9<Peripheral::Fmc>
{
	using Gpio = GpioE12;
...
template<>
struct GpioE12::Sck<Peripheral::Spi4>
{
	using Gpio = GpioE12;
...
#endif // MODM_STM32_GPIO_PIN_E12_HPP