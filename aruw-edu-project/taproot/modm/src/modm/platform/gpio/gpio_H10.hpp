...
#ifndef MODM_STM32_GPIO_PIN_H10_HPP
#define MODM_STM32_GPIO_PIN_H10_HPP
...
class GpioH10 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioH10>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioH10;
	using Input = GpioH10;
	using IO = GpioH10;
	using Type = GpioH10;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioH10::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim5),
			"GpioH10::Ch1 only connects to Tim5!");
	};
	template< Peripheral peripheral >
	struct D1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioH10::D1 only connects to Dcmi!");
	};
	template< Peripheral peripheral >
	struct D18 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioH10::D18 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioH10::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioH10;
...
template<>
struct GpioH10::Ch1<Peripheral::Tim5>
{
	using Gpio = GpioH10;
...
template<>
struct GpioH10::D1<Peripheral::Dcmi>
{
	using Gpio = GpioH10;
...
template<>
struct GpioH10::D18<Peripheral::Fmc>
{
	using Gpio = GpioH10;
...
#endif // MODM_STM32_GPIO_PIN_H10_HPP