...
#ifndef MODM_STM32_GPIO_PIN_F1_HPP
#define MODM_STM32_GPIO_PIN_F1_HPP
...
class GpioF1 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioF1>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioF1::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct A1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioF1::A1 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Scl { static void connect();
		static_assert(
			(peripheral == Peripheral::I2c2),
			"GpioF1::Scl only connects to I2c2!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioF1::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioF1;
...
template<>
struct GpioF1::A1<Peripheral::Fmc>
{
	using Gpio = GpioF1;
...
template<>
struct GpioF1::Scl<Peripheral::I2c2>
{
	using Gpio = GpioF1;
...
#endif // MODM_STM32_GPIO_PIN_F1_HPP