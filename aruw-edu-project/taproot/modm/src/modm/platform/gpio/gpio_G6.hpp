...
#ifndef MODM_STM32_GPIO_PIN_G6_HPP
#define MODM_STM32_GPIO_PIN_G6_HPP
...
class GpioG6 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioG6>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioG6::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct D12 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioG6::D12 only connects to Dcmi!");
	};
	template< Peripheral peripheral >
	struct Int2 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioG6::Int2 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioG6::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioG6;
...
template<>
struct GpioG6::D12<Peripheral::Dcmi>
{
	using Gpio = GpioG6;
...
template<>
struct GpioG6::Int2<Peripheral::Fmc>
{
	using Gpio = GpioG6;
...
#endif // MODM_STM32_GPIO_PIN_G6_HPP