...
#ifndef MODM_STM32_GPIO_PIN_D5_HPP
#define MODM_STM32_GPIO_PIN_D5_HPP
...
class GpioD5 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioD5>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioD5::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Nwe { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioD5::Nwe only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Tx { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart2),
			"GpioD5::Tx only connects to Usart2!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioD5::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioD5;
...
template<>
struct GpioD5::Nwe<Peripheral::Fmc>
{
	using Gpio = GpioD5;
...
template<>
struct GpioD5::Tx<Peripheral::Usart2>
{
	using Gpio = GpioD5;
...
#endif // MODM_STM32_GPIO_PIN_D5_HPP