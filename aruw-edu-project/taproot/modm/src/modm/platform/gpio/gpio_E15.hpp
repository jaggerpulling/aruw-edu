...
#ifndef MODM_STM32_GPIO_PIN_E15_HPP
#define MODM_STM32_GPIO_PIN_E15_HPP
...
class GpioE15 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioE15>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioE15;
	using Input = GpioE15;
	using IO = GpioE15;
	using Type = GpioE15;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioE15::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Bkin { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim1),
			"GpioE15::Bkin only connects to Tim1!");
	};
	template< Peripheral peripheral >
	struct D12 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE15::D12 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da12 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE15::Da12 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioE15::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioE15;
...
template<>
struct GpioE15::Bkin<Peripheral::Tim1>
{
	using Gpio = GpioE15;
...
template<>
struct GpioE15::D12<Peripheral::Fmc>
{
	using Gpio = GpioE15;
...
template<>
struct GpioE15::Da12<Peripheral::Fmc>
{
	using Gpio = GpioE15;
...
#endif // MODM_STM32_GPIO_PIN_E15_HPP