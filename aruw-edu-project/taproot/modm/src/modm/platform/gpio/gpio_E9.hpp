...
#ifndef MODM_STM32_GPIO_PIN_E9_HPP
#define MODM_STM32_GPIO_PIN_E9_HPP
...
class GpioE9 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioE9>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioE9::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim1),
			"GpioE9::Ch1 only connects to Tim1!");
	};
	template< Peripheral peripheral >
	struct D6 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE9::D6 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da6 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE9::Da6 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioE9::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioE9;
...
template<>
struct GpioE9::Ch1<Peripheral::Tim1>
{
	using Gpio = GpioE9;
...
template<>
struct GpioE9::D6<Peripheral::Fmc>
{
	using Gpio = GpioE9;
...
template<>
struct GpioE9::Da6<Peripheral::Fmc>
{
	using Gpio = GpioE9;
...
#endif // MODM_STM32_GPIO_PIN_E9_HPP