...
#ifndef MODM_STM32_GPIO_PIN_H11_HPP
#define MODM_STM32_GPIO_PIN_H11_HPP
...
class GpioH11 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioH11>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioH11;
	using Input = GpioH11;
	using IO = GpioH11;
	using Type = GpioH11;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioH11::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch2 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim5),
			"GpioH11::Ch2 only connects to Tim5!");
	};
	template< Peripheral peripheral >
	struct D19 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioH11::D19 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct D2 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioH11::D2 only connects to Dcmi!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioH11::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioH11;
...
template<>
struct GpioH11::Ch2<Peripheral::Tim5>
{
	using Gpio = GpioH11;
...
template<>
struct GpioH11::D19<Peripheral::Fmc>
{
	using Gpio = GpioH11;
...
template<>
struct GpioH11::D2<Peripheral::Dcmi>
{
	using Gpio = GpioH11;
...
#endif // MODM_STM32_GPIO_PIN_H11_HPP