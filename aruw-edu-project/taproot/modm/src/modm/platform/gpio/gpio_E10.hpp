...
#ifndef MODM_STM32_GPIO_PIN_E10_HPP
#define MODM_STM32_GPIO_PIN_E10_HPP
...
class GpioE10 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioE10>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioE10;
	using Input = GpioE10;
	using IO = GpioE10;
	using Type = GpioE10;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioE10::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch2n { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim1),
			"GpioE10::Ch2n only connects to Tim1!");
	};
	template< Peripheral peripheral >
	struct D7 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE10::D7 only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Da7 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioE10::Da7 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioE10::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioE10;
...
template<>
struct GpioE10::Ch2n<Peripheral::Tim1>
{
	using Gpio = GpioE10;
...
template<>
struct GpioE10::D7<Peripheral::Fmc>
{
	using Gpio = GpioE10;
...
template<>
struct GpioE10::Da7<Peripheral::Fmc>
{
	using Gpio = GpioE10;
...
#endif // MODM_STM32_GPIO_PIN_E10_HPP