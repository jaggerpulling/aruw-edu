...
#ifndef MODM_STM32_GPIO_PIN_H15_HPP
#define MODM_STM32_GPIO_PIN_H15_HPP
...
class GpioH15 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioH15>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioH15;
	using Input = GpioH15;
	using IO = GpioH15;
	using Type = GpioH15;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioH15::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch3n { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim8),
			"GpioH15::Ch3n only connects to Tim8!");
	};
	template< Peripheral peripheral >
	struct D11 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dcmi),
			"GpioH15::D11 only connects to Dcmi!");
	};
	template< Peripheral peripheral >
	struct D23 { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioH15::D23 only connects to Fmc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioH15::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioH15;
...
template<>
struct GpioH15::Ch3n<Peripheral::Tim8>
{
	using Gpio = GpioH15;
...
template<>
struct GpioH15::D11<Peripheral::Dcmi>
{
	using Gpio = GpioH15;
...
template<>
struct GpioH15::D23<Peripheral::Fmc>
{
	using Gpio = GpioH15;
...
#endif // MODM_STM32_GPIO_PIN_H15_HPP