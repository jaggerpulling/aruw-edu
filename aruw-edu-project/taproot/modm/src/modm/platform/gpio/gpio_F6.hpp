...
#ifndef MODM_STM32_GPIO_PIN_F6_HPP
#define MODM_STM32_GPIO_PIN_F6_HPP
...
class GpioF6 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioF6>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioF6::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim10),
			"GpioF6::Ch1 only connects to Tim10!");
	};
	template< Peripheral peripheral >
	struct In4 { static void connect();
		static_assert(
			(peripheral == Peripheral::Adc3),
			"GpioF6::In4 only connects to Adc3!");
	};
	template< Peripheral peripheral >
	struct Niord { static void connect();
		static_assert(
			(peripheral == Peripheral::Fmc),
			"GpioF6::Niord only connects to Fmc!");
	};
	template< Peripheral peripheral >
	struct Nss { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi5),
			"GpioF6::Nss only connects to Spi5!");
	};
	template< Peripheral peripheral >
	struct Rx { static void connect();
		static_assert(
			(peripheral == Peripheral::Uart7),
			"GpioF6::Rx only connects to Uart7!");
	};
	template< Peripheral peripheral >
	struct Sdb { static void connect();
		static_assert(
			(peripheral == Peripheral::Sai1),
			"GpioF6::Sdb only connects to Sai1!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioF6::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioF6;
...
template<>
struct GpioF6::Ch1<Peripheral::Tim10>
{
	using Gpio = GpioF6;
...
template<>
struct GpioF6::In4<Peripheral::Adc3>
{
	using Gpio = GpioF6;
...
template<>
constexpr int8_t
GpioF6::AdcChannel<Peripheral::Adc3> = 4;
template<>
struct GpioF6::Niord<Peripheral::Fmc>
{
	using Gpio = GpioF6;
...
template<>
struct GpioF6::Nss<Peripheral::Spi5>
{
	using Gpio = GpioF6;
...
template<>
struct GpioF6::Rx<Peripheral::Uart7>
{
	using Gpio = GpioF6;
...
template<>
struct GpioF6::Sdb<Peripheral::Sai1>
{
	using Gpio = GpioF6;
...
#endif // MODM_STM32_GPIO_PIN_F6_HPP