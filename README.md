...
```
cd my-amazing-project/aruw-edu-project
pipenv shell
# Build for hardware
scons build
# Run automated tests
...
1. Make sure you have VSCode opened in the folder `aruw-edu` (**not
   `aruw-edu-project`**)
...
1. Open the folder `aruw-aruw-edu-project` in VSCode. Hit the debug tab on the left side or type
...
`/aruw-edu-project/robot-type/robot_type.hpp` and change the macro defined in this file.
...
The below commands require that your working directory is `/aruw-edu-project` (where the
